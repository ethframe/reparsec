...
def continue_parse(
    ...
    for r in ra.repairs:
        rb = parse(r.value, stream, r.pos, r.ctx, (r.cap,))
        # Merge
        if type(rb) is Ok:
            reps.append(
                Repair(
                    r.cost, r.skip, rb.ctx.rm[0] if rb.consumed else r.cap,
            ...
            for rr in rb.repairs:
                reps.append(
                    Repair(
                        r.cost + rr.cost, r.skip, rr.cap, _join_ops(r, rr),
...
def join_repairs(
    ...
    if min_skip is not None:
        reps.extend(
            r for r in rb.repairs if r.skip is not None and r.skip < min_skip
        )
        if rb.min_skip is not None and rb.min_skip < min_skip:
            min_skip = rb.min_skip
    else:
        reps.extend(r for r in rb.repairs if r.skip is not None)
        min_skip = rb.min_skip
    if ra.consumed:
