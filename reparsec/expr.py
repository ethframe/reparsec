...
def infix_left(
        arg: Parser[S, P, V], op: Parser[S, P, U],
        fn: Callable[[U, V, V], V]) -> Parser[S, P, V]:
    return (
        arg + (op + arg).many()
...
def prefix(
        op: Parser[S, P, U], arg: Parser[S, P, V],
        fn: Callable[[U, V], V]) -> Parser[S, P, V]:
    return (
        op.many() + arg
...
def postfix(
        arg: Parser[S, P, V], op: Parser[S, P, U],
        fn: Callable[[V, U], V]) -> Parser[S, P, V]:
    return (
        arg + op.many()
